...
/**
 * The snake high-score board.
 *
 * Same row grammar as the trivia board — repeating rank numerals on a tie, gold
 * for #1, your own row tinted — so the two read as siblings even though they
 * rank different things and share no code. What differs is the number: one
 * score, not a correct-of-answered ratio, with the play count dimmed beside it.
 */
export function SnakeBoard({ rows, loading, myId }: SnakeBoardProps) {
  if (loading) {
    return (
      <Stack spacing={0.5}>
